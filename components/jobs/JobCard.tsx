...
function JobCardComponent({ job }: { job: Job }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-white/[0.09] bg-gradient-to-b from-white/[0.06] to-white/[0.028] p-6 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] backdrop-blur-md transition-colors hover:border-white/[0.12] hover:from-white/[0.065] hover:to-white/[0.034]">
      <div
        aria-hidden="true"
        className={cn(
      ...
      <div className="relative space-y-4">
        ...
        <div className="space-y-3.5">
          ...
          {arrangement ? (
            <div>
              ...
              <div className="mt-1.5 text-[15px] font-medium leading-snug text-white/78">{arrangement}</div>
            </div>
          ) : job.type && job.type !== "—" ? (
            <div>
              <div className="text-[11px] font-medium uppercase tracking-[0.09em] text-white/55">
              ...
              <div className="mt-1.5 text-[15px] font-medium leading-snug text-white/78">{job.type}</div>
            </div>
          ) : null}

          {job.salary ? (
            <div>
              <div className="text-[11px] font-medium uppercase tracking-[0.09em] text-white/55">{t("labelSalary")}</div>
              <div className="mt-1.5 text-[15px] font-semibold tabular-nums leading-snug text-white/85">{job.salary}</div>
            </div>
          ) : null}

          {posted ? (
            <div>
              <div className="text-[11px] font-medium uppercase tracking-[0.09em] text-white/55">{t("labelPosted")}</div>
        ...
        {job.tags.length ? (
          <div className="space-y-2.5 pt-0.5">
            <div className="text-[11px] font-medium uppercase tracking-[0.09em] text-white/55">{t("labelSignals")}</div>
