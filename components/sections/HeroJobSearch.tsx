...
type Props = {
  ...
  /** `split` = hero left column — stack until wide enough so nothing clips. */
  layout?: "full" | "split";
};

const fieldClass = cn(
  SITE_HERO_SEARCH_HEIGHT,
  "min-w-0 rounded-none border-0 bg-transparent text-base text-white shadow-none placeholder:text-white/38 focus:bg-transparent focus-visible:rounded-none focus-visible:outline-none lg:text-[1.0625rem]",
);

export function HeroJobSearch({ quickFilters, layout = "full" }: Props) {
      ...
      <div className="relative min-w-0">
        ...
        >
          ...
          >
            <label
              className={cn(
                "relative flex min-w-0 items-center border-b border-white/[0.08] transition-colors focus-within:bg-white/[0.03]",
                SITE_HERO_SEARCH_HEIGHT,
                split
                  ? "xl:border-b-0 xl:border-r xl:border-white/[0.08]"
                  : "lg:border-b-0 lg:border-r lg:border-white/[0.08]",
              )}
            >
            ...
            <Button
              ...
              className={cn(
                SITE_HERO_SEARCH_HEIGHT,
                "w-full min-w-0 rounded-none px-5 text-pretty text-base font-semibold focus-visible:ring-inset focus-visible:ring-offset-0 sm:px-6",
                split
                  ? "xl:min-w-[12.5rem] xl:px-6 xl:text-[1.0625rem]"
                  : "lg:min-w-[15rem] lg:px-7 lg:text-[1.0625rem] xl:min-w-[16rem]",
              )}
            >
              {t("searchSubmit")}
            </Button>
