...
function FacetGroup({
  ...
  return (
    <div className="rounded-xl border border-border bg-[#f8fafc] p-4">
      <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-2">{title}</div>
      {values.length ? (
        ...
        <p className="mt-2 text-sm text-muted-2">{emptyHint}</p>
      ) : null}
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-border bg-[#f8fafc] p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="text-[15px] font-medium text-foreground">{t("searchTitle")}</div>
            <div className="mt-1.5 text-sm leading-snug text-muted-2">{t("searchSubtitle")}</div>
            <p className="mt-2.5 text-sm text-body" aria-live="polite">
              <span className="text-muted">{t("foundCount", { count: totalCount })}</span>
              {activeCount > 0 ? (
                <span className="text-muted-2"> · {t("activeCount", { count: activeCount })}</span>
              ) : null}
            </p>
          </div>
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted-2">{t("privacyNote")}</p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[320px_1fr] lg:items-start lg:gap-9">
        <div
          className={cn(
            mobileOpen
              ? "fixed inset-0 z-[90] flex flex-col bg-white lg:static lg:z-auto lg:block lg:bg-transparent"
              : "hidden lg:sticky lg:top-24 lg:block lg:self-start",
          )}
        >
          {mobileOpen ? (
            <div className="flex items-center justify-between border-b border-border px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] lg:hidden">
              <div className="text-[15px] font-medium text-foreground">
                {activeCount ? t("filtersWithCount", { count: activeCount }) : t("filters")}
              </div>
              <button
                type="button"
                className="inline-flex h-11 w-11 items-center justify-center rounded-xl text-muted-2 hover:bg-[#f5f7fb]"
          ...
          <div className="rounded-none border-0 bg-transparent p-4 lg:rounded-2xl lg:border lg:border-border lg:bg-[#141018] lg:p-5">
            <div className="hidden items-center justify-between gap-3 border-b border-border pb-4 lg:flex">
              <div className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-2">
                {t("filters")}
              </div>
              <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-2">
            ...
            <div className="mt-5 space-y-6">
              <div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-medium tracking-wide text-muted-2" htmlFor="exp-min">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-medium tracking-wide text-muted-2" htmlFor="exp-max">
              ...
              <div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-medium tracking-wide text-muted-2" htmlFor="hours-min">
                      {t("desiredHoursMin")}
                    </label>
                    <Input
                      id="hours-min"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-medium tracking-wide text-muted-2" htmlFor="hours-max">
                      {t("desiredHoursMax")}
                    </label>
                    <Input
                      id="hours-max"
                ...
                <p className="mt-2 text-xs text-muted-2">{t("desiredHoursHint")}</p>
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-xs leading-relaxed text-muted-2">{t("workplaceNeedsHint")}</p>
              </div>
          ...
          {mobileOpen ? (
            <div className="border-t border-border space-y-2 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] lg:hidden">
              {activeCount > 0 ? (
        ...
        <div className="space-y-3">
          {candidates.map((c) => {
            return (
              <div key={c.id} className="rounded-3xl border border-border bg-[#f8fafc] p-5">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium text-foreground/80">{c.displayName}</div>
                    <div className="mt-1 text-xs text-muted-2">
                      {(c.profileTitle || "").trim() ? c.profileTitle : c.experienceLevel}{" "}
                      {c.location ? <span className="text-muted-2">·</span> : null} {c.location}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {c.certificates.length > 2 ? (
                    <div className="text-[11px] text-muted-2">+{c.certificates.length - 2}</div>
                  ) : null}
                  {!c.certificates.length && c.hasBLicense ? (
                    <span className="rounded-full border border-border bg-[#f8fafc] px-3 py-1 text-xs text-body">
                      {certificateLabel}
                    </span>
                  ) : null}
                  {c.seekingFirstJob ? (
                    <span className="rounded-full border border-border bg-[#f8fafc] px-3 py-1 text-xs text-muted">
                      {t("seekingFirstJob")}
                    </span>
                  ) : null}
                </div>
                ...
                {c.skills.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {c.skills.slice(0, 6).map((skill) => (
                      <span
                        key={skill}
                        className="rounded-full border border-border bg-[#f8fafc] px-3 py-1 text-xs text-body"
                      >
                        {skill}
                      </span>
                    ))}
