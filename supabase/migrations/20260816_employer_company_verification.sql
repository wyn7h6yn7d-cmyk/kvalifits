-- Employer company verification fields (no Äriregister integration yet).
-- Entering a company name must NEVER mark the company verified.
-- Non-admins cannot change verification columns (trigger).
...
-- Backfill + normalize before NOT NULL / check.
...
where verification_status is null
   ...
   or company_verified is distinct from (verification_status = 'verified');

-- Keep boolean aligned with status for any rows already consistent.
update public.employer_profiles
...
begin
  ...
  if not coalesce(is_admin, false) then
    if tg_op = 'INSERT' then
      ...
      -- Employers (and anon) cannot self-verify by writing these columns.
  ...
  if new.verification_status = 'verified' then
    ...
    new.verified_at := null;
    -- Keep source only while verified; clear otherwise (no fake “verified company” signal).
    new.verification_source := null;
  end if;
...
-- Admins may list and update any employer profile (verification moderation).
