...
-- Seekers may insert/select/delete only their own rows.
-- Employers have no SELECT — they cannot see who saved a listing.
-- anon has no table grants — guests cannot persist bookmarks.
...
-- ---------------------------------------------------------------------------
-- Helpers (SECURITY DEFINER + row_security off: no RLS recursion / metadata)
-- ---------------------------------------------------------------------------
...
comment on function public.current_user_is_seeker() is
  'True when auth.uid() has profiles.role = seeker. SECURITY DEFINER to avoid RLS recursion.';
...
comment on function public.current_user_has_saved_job(uuid) is
  'True when the current user has a saved_jobs row for this job. Does not reveal other users.';
...
comment on function public.current_user_saved_job_for_employer(uuid) is
  'True when the current seeker saved a job belonging to this employer profile.';
...
-- No UPDATE policy. No employer / anon / admin SELECT on saved_jobs.
...
-- ---------------------------------------------------------------------------
-- Allow a seeker to read job_posts (incl. archived) they already saved,
-- so the saved-jobs page can show inactive listings. Cannot be used to
-- probe unpublished drafts: INSERT only allows currently published jobs.
-- ---------------------------------------------------------------------------
...
-- Company name on saved (possibly archived) listings.
