-- Schema/RLS reconciliation part 2 of 3.
-- Catch-up for remotes missing search RPCs, taxonomy catalog, and archive function.
-- Idempotent (CREATE OR REPLACE / IF NOT EXISTS / ON CONFLICT).
-- pg_cron schedule is skipped when the extension is unavailable.
...
-- ===== 20260817193000_published_job_facet_values.sql =====
...
begin
  ...
  if v_facet = 'title' then
    ...
    from (
      ...
      and (
        v_q is null
        or public.job_search_norm(replace(lower(coalesce(jp.job_type::text, '')), '-', '_'))
    ...
    from (
      ...
      and (
        v_q is null
        or public.job_search_norm(replace(replace(lower(coalesce(jp.work_type::text, '')), '-', '_'), 'onsite', 'on_site'))
          like '%' || public.job_search_norm(v_q) || '%'
      )
      group by 1
      having count(*) > 0
...
begin
  ...
  if v_facet = 'title' then
    ...
    from (
      ...
      and (
        v_q is null
        or public.job_search_norm(replace(lower(coalesce(jp.job_type::text, '')), '-', '_')) like '%' || v_qn || '%'
      )
      group by 1
      having count(*) > 0
    ) x;
