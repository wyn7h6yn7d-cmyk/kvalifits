-- Fixes: last status timestamp + append-only employer status trail.
-- Mirror of migration 20260817115759_job_application_status_audit.sql.
-- Run in Supabase SQL Editor if status_updated_at / history is missing.
-- Safe to re-run. Does not expose events or notes to candidates.
